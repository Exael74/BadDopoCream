...
public class GameLogic {
    ...
    private void movePlayerEntity(Player player, Direction direction) {
        ...
        // Check for Active Fogata Collision
        Fogata fogata = getFogataAt(player.getPosition());
        if (fogata != null && fogata.isActive()) {
            player.die();
            return;
        }
    ...
    public Point findRandomEmptyPosition() {
        ...
        for (int x = 0; x < gridSize; x++) {
            for (int y = 0; y < gridSize; y++) {
                ...
                if (collisionDetector.isPositionFree(pos) && !isHotTile(pos) && !hasFogataAt(pos)) {

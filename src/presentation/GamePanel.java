...
public class GamePanel extends JPanel implements java.awt.event.ActionListener {
    ...
    protected void paintComponent(Graphics g) {
        ...
        drawFogatas(g2d, offsetX, offsetY);
    ...
    /**
     * Dibuja todas las fogatas.
     */
    private void drawFogatas(Graphics2D g2d, int offsetX, int offsetY) {
        for (FogataSnapshot fogata : gameFacade.getFogataSnapshots()) {
            Point pos = fogata.getPosition();
            int drawSize = 40; // Similar to ice/fruit

            int x = offsetX + pos.x * CELL_SIZE + (CELL_SIZE - drawSize) / 2;
            int y = offsetY + pos.y * CELL_SIZE + (CELL_SIZE - drawSize) / 2;

            // Draw Campfire base
            if (resources.campfireGif != null) {
                g2d.drawImage(resources.campfireGif.getImage(), x, y, drawSize, drawSize, this);
            }

            // Draw Flame if Active
            if (fogata.isActive() && resources.flameGif != null) {
                // Flame might need to be shifted up or same position?
                // "mostrar el gif de campfire y flame al tiempo, en la misma posicion"
                g2d.drawImage(resources.flameGif.getImage(), x, y, drawSize, drawSize, this);
            }
        }
    }

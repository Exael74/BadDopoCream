...
public class ScreenManager {
    ...
    // ==================== PANTALLA 5: LOAD GAME (OPTIONS) ====================

    private void showLoadGameMenu() {
        java.util.List<String> savedGames = domain.GameFacade.listSavedGames();

        if (savedGames.isEmpty()) {
            JOptionPane.showMessageDialog(parentWindow, "No hay partidas guardadas.", "Cargar Partida",
        ...
        if (selectedSave != null) {
            try {
                // Load Game
                domain.GameFacade loadedFacade = domain.GameFacade.loadFromSave(selectedSave);
                ...
                // Launch Game Window
                SwingUtilities.invokeLater(() -> {
                    new GameWindow(loadedFacade, resources); // GameWindow constructor handles setup
                });

            } catch (Exception e) {
                domain.BadDopoLogger.logError("Error al cargar partida: " + e.getMessage(), e);
                JOptionPane.showMessageDialog(parentWindow, "Error al cargar la partida: " + e.getMessage(), "Error",
